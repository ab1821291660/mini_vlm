...
def parse_reasoning_answer(text):
    ...
    reasoning_text = reasoning_match.group(1).strip() if reasoning_match else None#None
    answer_text = answer_match.group(1).strip() if answer_match else None#None
    ...
    if 'model' in args.load_from:
        ...
        if not os.path.exists(ckp):
            raise FileNotFoundError(
                f"未找到权重文件: {ckp}\n"
                f"请确认路径后重试。示例命令:\n"
                f"1) python eval_vlm.py --load_from model --weight cot_sft --device cuda\n"
                f"2) python eval_vlm.py --load_from model --weight cot_sft --cot_weight_path /root/autodl-tmp/minimind-v/minimind-v/out/cot_sft_768.pth --device cuda\n"
                f"3) python eval_vlm.py --load_from model --weight cot_sft --cot_force_reasoning 0 --device cuda"
            )
        model = MiniMindVLM(
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            '说明:\n'
            '- cot_sft 权重必须存在，否则会报错。\n'
            '- cot_force_reasoning 默认启用 Reasoning -> Answer 模板。\n'
            '- open_thinking 默认在 cot_sft 下为 1，用户显式传参可覆盖。\n'
            '- cot_sft 默认使用更稳的解码参数，用户传 --temperature/--top_p/--max_new_tokens 可覆盖。\n'
            '- 若输出解析不到 Reasoning/Answer，将 fallback 打印完整文本。\n\n'
            '示例:\n'
            '1. python eval_vlm.py --load_from model --weight cot_sft --device cuda\n'
            '2. python eval_vlm.py --load_from model --weight cot_sft --cot_weight_path /root/autodl-tmp/minimind-v/minimind-v/out/cot_sft_768.pth --device cuda\n'
            '3. python eval_vlm.py --load_from model --weight cot_sft --cot_force_reasoning 0 --device cuda'
        ),
    )
    ...
    # parser.add_argument('--weight', default='cot_sft', type=str, choices=['pretrain_vlm', 'sft_vlm', 'cot_sft'], help='权重名称前缀（pretrain_vlm, sft_vlm, cot_sft）',)##===================================
    parser.add_argument('--weight', default='sft_vlm', type=str, choices=['pretrain_vlm', 'sft_vlm', 'cot_sft'], help='权重名称前缀（pretrain_vlm, sft_vlm, cot_sft）',)##===================================
    parser.add_argument('--cot_weight_path', default='./out/cot_sft_768.pth', type=str, help='cot_sft 权重路径（仅 weight=cot_sft 时生效）')##===================================
    parser.add_argument('--cot_force_reasoning', default=1, type=int, choices=[0, 1], help='cot_sft 是否默认启用 Reasoning -> Answer 模板（0=否，1=是）')##===================================
    parser.add_argument('--open_thinking',
                        default=None,##========
                        type=int,
                        choices=[0, 1],
                        help='是否开启自适应思考（0=否，1=是，默认: cot_sft=1，其他=0）')##===================================##===================================##===================================##===================================
    parser.add_argument('--cot_temperature', default=0.35, type=float, help='cot_sft 默认温度（未手动传 temperature 时生效）')
    parser.add_argument('--cot_top_p', default=0.8, type=float, help='cot_sft 默认 top_p（未手动传 top_p 时生效）')
    parser.add_argument('--cot_max_new_tokens', default=256, type=int, help='cot_sft 默认最大生成长度（未手动传 max_new_tokens 时生效）')
    ...
    is_cot_weight = args.weight == 'cot_sft'##========true
    effective_open_thinking = args.open_thinking if args.open_thinking is not None    else (1 if is_cot_weight else 0)##1 ##========
    ##
    effective_temperature = args.temperature
    effective_top_p = args.top_p
    effective_max_new_tokens = args.max_new_tokens
    if is_cot_weight:##========true
    ...
    for image_file in sorted(os.listdir(args.image_dir)):
        if image_file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp')):
            ...
            image = Image.open(image_path).convert('RGB')#512-512
            pixel_values = {k: v.to(args.device) for k, v in MiniMindVLM.image2tensor(image, preprocess).items()}#1-3-256-256##===================================
            ##
            ...
            if is_cot_weight:
                ...
                reasoning_text, answer_text, _ = parse_reasoning_answer(final_text)#None  #None##===================================
