...
### 2' 数据下载
**Pretrain+SFT阶段（minimind-v 原项目数据集）：**
从[数据集链接](https://huggingface.co/datasets/jingyaogong/minimind-v_dataset)下载所需内容并放到`./dataset`下。
...
```
若无多余空间存放pretrain数据，可尝试跳过pretrain训练步骤直接进行sft训练。
...
CoT SFT数据蒸馏：
这个阶段的数据集是通过将原项目中SFT数据进行蒸馏得到，具体实现见dataset/distilled_sft_i2t.py 这个蒸馏过程需要调用外部api，对数据生成解释，比较耗时。
```bash
export DISTILL_API_KEY="你的api key"
python dataset/distilled_sft_i2t.py
``` 

RL阶段数据： 这里使用的是[Innovator-VL-RL-172K](
...
### 3' 开始训练
这里为了适配推理任务，LLM部分使用的是minimind官方提供的reasoning权重（在out/llm_768.pth），训练分为4个阶段：
...
```bash
python train_sft_vlm.py --epochs 2 --from_weight sft_vlm --data_path ../dataset/sft_i2t_cot_distilled.parquet --save_weight cot_sft
```
...
```bash
python train_grpo_vlm.py       cot_grpo
python train_grpo_vlm.py --epochs 2 --from_weight sft_vlm_cot --data_path ../dataset/Innovator-VL-RL-172K.parquet --save_weight cot_grpo
```
...
##
# 测试SFT CoT模型
python eval_vlm.py --weight cot_sft
# 测试RL模型
python eval_vlm.py --weight cot_grpo
```
